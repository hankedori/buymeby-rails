class VendorInventoryParser
  ...
  def perform
    ...
    JSON.parse(response)["module_data"]["items"].each do |what|
      vendor.items.create!(name: what["name"], price: what["pricing"]["price"].tr!('$', '').to_f, unit: what["pricing"]["unit"] || "each", quantity: 10, description: what["size_aria"] || what["name"], image_src: what["image"]["url"], category: category)
    end
  end
end
